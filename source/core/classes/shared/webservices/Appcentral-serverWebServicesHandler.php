...
class AppcentralServerWebServicesHandler extends \Innomatic\Webservices\WebServicesHandler {
	public static function list_available_repositories() {
		...
		$rep_query = \Innomatic\Core\InnomaticContainer::instance('\Innomatic\Core\InnomaticContainer')->getDataAccess()->execute('SELECT appcentral_reps.id AS id, appcentral_reps.name AS name, appcentral_reps.description AS description FROM appcentral_reps_access,appcentral_reps WHERE appcentral_reps_access.profileid='.\Innomatic\Core\InnomaticContainer::instance('\Innomatic\Core\InnomaticContainer')->getWebServicesProfile().' AND appcentral_reps_access.repositoryid=appcentral_reps.id ORDER BY name');
